...
function clearValidationErrors(){
  ['nickname','location','styleTag','moodNote','dateSpotted'].forEach(k=>{
    const e=$('err-'+k.replace(/([a-z])([A-Z])/g,'$1-$2').toLowerCase());if(e)e.textContent='';
    const i=$('input-'+k.replace(/([a-z])([A-Z])/g,'$1-$2').toLowerCase())||$('select-style-tag');
...
function handleFormSubmit(e){
  ...
  const raw={
    nickname:$('input-nickname')?.value,location:$('input-location')?.value,
    styleTag:$('select-style-tag')?.value,moodNote:$('input-mood-note')?.value,
    dateSpotted:$('input-date-spotted')?.value
  };
  const v=validateFontEntry(raw);
  if(!v.isValid){
    let first=null;
    const map=[['nickname','err-nickname',$('input-nickname')],['location','err-location',$('input-location')],['styleTag','err-style-tag',$('select-style-tag')],['moodNote','err-mood-note',$('input-mood-note')],['dateSpotted','err-date-spotted',$('input-date-spotted')]];
