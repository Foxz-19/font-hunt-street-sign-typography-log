...
export function renderGrid(f,af,tc){
  ...
  if(tc===0)return`<div class="empty-state" role="status"><div class="empty-state-icon" aria-hidden="true">&#9998;</div><h3 class="empty-state-title">No Field Notes Yet</h3><p class="empty-state-text">Your notebook is empty. Look up at shopfronts and awnings, then log your first find.</p><div class="empty-state-actions"><button type="button" class="btn btn-primary" data-action="load-samples">Load Sample Finds</button></div></div>`.trim();
